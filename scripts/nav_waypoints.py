...
def talker(coordinates):
	...
	pkg_path = str(rospack.get_path('nav_waypoints')) + "/data/waypoints.csv"
	f = open(pkg_path,'r')
	array = PoseArray()
	...
	for goal in f:
        ...
        #To not have to deal with threading, Im gonna publish just a couple times in the beginning, and then continue with telling the robot to go to the points
	count = 0
	while count<1:
		rate.sleep()	
		pub.publish(array)
